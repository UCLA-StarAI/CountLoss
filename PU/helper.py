...
class UnlabelData(torch.utils.data.Dataset): 
    def __init__(self, transform=None, target_transform=None, pos_data=None, \
        ...
        self.true_targets = np.concatenate((np.ones(pos_data.shape[0],  dtype= np.int_), np.zeros(neg_data.shape[0],  dtype= np.int_)), axis=0)
        self.targets = np.zeros_like(self.true_targets, dtype= np.int_)
...
def get_dataset_val(data_dir, data_type,net_type, device, alpha, beta, batch_size, val_percent=0.1): 
    ...
    if data_type=='gaussian': 
        ...
        traindata = MNIST17Data(root=data_dir, train=True, transform=transform_train)
        testdata = MNIST17Data(root=data_dir, train=False, transform=transform_test)
        train_size = int(3000*(1- val_percent))
        total_pos_used = int(train_size + train_size*beta)
        total_neg_used = int(train_size*(1 - beta))
        val_size = int(3000*(val_percent))

        p_traindata, u_traindata = get_PUDataSplits(traindata, pos_size=train_size, alpha=alpha, beta=beta,data_type='mnist')
        #new train: 2400, 1200, 1200
        #pos used = 2400 + 1200, ne used = 1200
        #val: 600, 300, 300
        #3600 --> 4500, 1200 -> 1500
        p_validdata, u_validdata = PosData(transform=transform_train, target_transform=traindata.target_transform,\
                                    data=traindata.p_data[total_pos_used: total_pos_used + val_size], index=np.array(range(val_size)), data_type='mnist'),\
                                  UnlabelData(transform=transform_train, \
                                    target_transform=traindata.target_transform, \
                                    pos_data=traindata.p_data[total_pos_used + val_size: int(total_pos_used + val_size + beta*val_size)], neg_data=traindata.n_data[total_neg_used:int(total_neg_used + (1 - beta)*val_size)], \
                                    index=np.array(range(val_size)),data_type='mnist')
        p_testdata, u_testdata = get_PUDataSplits(testdata, pos_size=500, alpha=alpha, beta=beta,data_type='mnist')
        ...
        #Before pos: 15000 + 7500 = 22500 neg: 7500 
        train_size = int(15000*(1- val_percent)) #12000
        total_pos_used = int(train_size + train_size*beta) #12000 + 6000 = 18000
        total_neg_used = int(train_size*(1 - beta)) #6000

        val_size = int(15000*(val_percent)) #3000 pos, 1500 pos - 1500 neg
        ...
        p_validdata, u_validdata = PosData(transform=transform_train, target_transform=traindata.target_transform,\
                                  ...
                                  UnlabelData(transform=transform_train, \
                                    ...
                                    pos_data=traindata.p_data[total_pos_used + val_size: int(total_pos_used + val_size + beta*val_size)], neg_data=traindata.n_data[total_neg_used:int(total_neg_used + (1 - beta)*val_size)], \
                                    index=np.array(range(val_size)),data_type='mnist')
        p_testdata, u_testdata = get_PUDataSplits(testdata, pos_size=2500, alpha=alpha, beta=beta,data_type='mnist')
        ...
        train_size = int(2500*(1- val_percent))
        total_pos_used = int(train_size + train_size*beta)
        total_neg_used = int(train_size*(1 - beta))
        val_size = int(2500*(val_percent))

        p_traindata, u_traindata = get_PUDataSplits(traindata, pos_size=train_size, alpha=alpha, beta=beta,data_type='cifar')
        p_validdata, u_validdata = PosData(transform=transform_train, target_transform=traindata.target_transform,\
                                    data=traindata.p_data[total_pos_used: total_pos_used + val_size], index=np.array(range(val_size)), data_type='cifar'),\
                                  UnlabelData(transform=transform_train, \
                                    target_transform=traindata.target_transform, \
                                    pos_data=traindata.p_data[total_pos_used + val_size: int(total_pos_used + val_size + beta*val_size)], neg_data=traindata.n_data[total_neg_used:int(total_neg_used + (1 - beta)*val_size)], \
                                    index=np.array(range(val_size)),data_type='cifar')
        p_testdata, u_testdata = get_PUDataSplits(testdata, pos_size=500, alpha=alpha, beta=beta,data_type='cifar')
        ...
        train_size = int(12500*(1- val_percent))
        total_pos_used = int(train_size + train_size*beta)
        total_neg_used = int(train_size*(1 - beta))
        val_size = int(12500*(val_percent))
        ...
        p_validdata, u_validdata = PosData(transform=transform_train, target_transform=traindata.target_transform,\
                                  ...
                                  UnlabelData(transform=transform_train, \
                                    ...
                                    pos_data=traindata.p_data[total_pos_used + val_size: int(total_pos_used + val_size + beta*val_size)], neg_data=traindata.n_data[total_neg_used:int(total_neg_used + (1 - beta)*val_size)], \
                                    index=np.array(range(val_size)),data_type='cifar')
        p_testdata, u_testdata = get_PUDataSplits(testdata, pos_size=2500, alpha=alpha, beta=beta,data_type='cifar')
    ...
    return p_trainloader, u_trainloader, p_validloader, u_validloader, p_testloader, u_testloader, net, X, Y, p_validdata, u_validdata, u_traindata